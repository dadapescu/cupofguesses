...
// Sterge pariurile (commits + reveals) tuturor utilizatorilor.
// Rezultatele meciurilor raman neatinse.
export async function resetAllBets() {
  const snap = await getDocs(usersCol());
  await Promise.all(
    snap.docs.map((d) =>
      setDoc(userRef(d.id), { commits: {}, reveals: {} }, { merge: true })
    )
  );
}
