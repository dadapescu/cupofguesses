...
export default function App() {
  ...
  const [funFacts, setFunFacts] = useState([]); // [{id, text, author, ts}]
  ...
  const placeBet = async (matchId, pick) => {
    ...
    if (new Date(match.kickoff).getTime() - 60 * 60 * 1000 <= Date.now()) return showToast("Pariurile s-au inchis cu 1h inainte de meci.");
    if (myPicks[matchId]) return showToast("Ai pariat deja. Biletul e sigilat - nu te poti razgandi.");
    const cloudCommits = (usersData[me] && usersData[me].commits) || {};
    if (cloudCommits[matchId]) return showToast("Ai pariat deja de pe alt device.");
    const nextPicks = { ...myPicks, [matchId]: pick };
    const myCommits = { ...cloudCommits, [matchId]: true };
  ...
  const handleResetBets = async () => {
    try {
      ...
      setMyPicks({});
      LS.set("mypicks", {});
      showToast("Pariurile au fost resetate. Toata lumea porneste de la 0.");
    } catch (e) {
      console.error(e);
  ...
  const addFunFact = async (text) => {
    if (!text.trim()) return;
    const next = [
      { id: "ff" + Date.now(), text: text.trim(), author: me, ts: new Date().toISOString() },
      ...funFacts,
    ];
    setFunFacts(next);
    try { await saveState({ funFacts: next }); }
    catch { showToast("Eroare la salvare."); }
  };

  const removeFunFact = async (id) => {
    const next = funFacts.filter((f) => f.id !== id);
    setFunFacts(next);
    try { await saveState({ funFacts: next }); }
    catch { showToast("Eroare la salvare."); }
  };
  ...
  const standings = users
    .map((u) => {
      ...
      finished.forEach((m) => {
        ...
        if (rev) {
          played++;
          const ok = rev === m.result.pick;
          if (ok) pts++;
          hits.push({ match: m, pick: rev, ok });
        }
      });
      return { name: u, pts, played, sealed, hits };
    })
  ...
  // Urmatorul meci: cel mai apropiat in viitor, fara echipe-placeholder
  const nextMatch = allMatches
    .filter((m) => !m.isPlaceholder && new Date(m.kickoff).getTime() > now)
        ...
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline" }}>
            <div>
              ...
              <div style={{ fontSize: 11, letterSpacing: "0.14em", opacity: 0.65, textTransform: "uppercase", marginTop: 4, fontStyle: "italic" }}>
                by fire alarm enthusiasts
              </div>
            </div>
          ...
          {nextMatch && <NextMatchBanner match={nextMatch} now={now} />}
...
function NextMatchBanner({ match, now }) {
  const ms = new Date(match.kickoff).getTime() - now;
  ...
  const imminent = totalSec < 3600 && totalSec > 0;

  // Fiecare segment de timp e intr-un container cu latime fixa,
  // ca sa nu se mute nimic cand cifrele se schimba.
  const Seg = ({ num, label }) => (
    <span style={{ display: "inline-flex", flexDirection: "column", alignItems: "center", minWidth: 38 }}>
      <span style={{
        fontFamily: "'Saira Condensed', monospace", fontSize: 22, fontWeight: 800,
        color: imminent ? C.gold : "#fff", lineHeight: 1, fontVariantNumeric: "tabular-nums",
      }}>{num}</span>
      <span style={{ fontSize: 9, opacity: 0.6, letterSpacing: "0.1em", textTransform: "uppercase", marginTop: 2 }}>
        {label}
      </span>
    </span>
  );
  ...
  return (
    <div style={{
      ...
      background: imminent ? "rgba(217, 154, 27, 0.18)" : "rgba(255,255,255,0.08)",
      border: `1px solid ${imminent ? "rgba(217, 154, 27, 0.5)" : "rgba(255,255,255,0.12)"}`,
      display: "flex", flexDirection: "column", alignItems: "center", gap: 8,
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "center", textAlign: "center" }}>
        <span style={{ fontSize: 10, letterSpacing: "0.16em", opacity: 0.75, textTransform: "uppercase", fontWeight: 700 }}>
          {imminent ? "🔥 Incepe in" : "Urmeaza"}
        </span>
        <span style={{ fontSize: 14, fontWeight: 700 }}>
          {flag(match.home)} {match.home} <span style={{ opacity: 0.5 }}>vs</span> {match.away} {flag(match.away)}
        </span>
      </div>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 6 }}>
        {days > 0 && <Seg num={days} label="zile" />}
        <Seg num={hh} label="ore" />
        <Seg num={mm} label="min" />
        <Seg num={ss} label="sec" />
      </div>
    </div>
  );
}
...
function MatchCard({ match, me, myPicks, usersData, users, now, onPick }) {
  ...
  const started = !placeholder && new Date(match.kickoff).getTime() - 60 * 60 * 1000 <= now;
  const done = !!match.result;
  const myPick = myPicks[match.id] || null;
  // Verifica si in cloud daca am pariat de pe alt device
  const cloudHasCommit = !!((usersData[me] && usersData[me].commits) || {})[match.id];
  ...
  const alreadyBet = !!myPick || cloudHasCommit;
  // Daca avem pariu pe acest device, il aratam. Daca nu, dar e in cloud, aratam "alt device".
  const onOtherDevice = cloudHasCommit && !myPick;
  const PickBtn = ({ val, label }) => {
    const active = myPick === val;
    const disabled = started || placeholder || alreadyBet;
    return (
      <button onClick={() => onPick(match.id, val)} disabled={disabled}
        style={{ flex: 1, padding: "12px 0", borderRadius: 10, fontSize: 15, fontWeight: 700, cursor: disabled ? "default" : "pointer", border: `2px solid ${active ? C.navy : C.line}`, background: active ? C.navy : "#fff", color: active ? "#fff" : disabled ? C.line : C.ink, transition: "all .15s", opacity: alreadyBet && !active ? 0.4 : 1 }}>
        <div>{val}</div>
      ...
      {!done && !placeholder && (
        <>
          ...
          <div style={{ marginTop: 10, fontSize: 12, color: C.inkSoft, display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
            ...
            {myPick ? (
              <span style={{ color: C.green, fontWeight: 700 }}>🔒 Biletul tau: {myPick} (sigilat)</span>
            ) : onOtherDevice ? (
              <span style={{ color: C.green, fontWeight: 700 }}>🔒 Ai pariat de pe alt device</span>
            ) : null}
          </div>
        </>
      )}
  ...
  return (
    <div style={st.card}>
      ...
      <p style={{ color: C.inkSoft, fontSize: 13, margin: "4px 0 16px" }}>
        1 punct per pronostic corect · {finishedCount} {finishedCount === 1 ? "meci jucat" : "meciuri jucate"} · apesi pe un coleg ca sa-i vezi biletele
      </p>
      {standings.length === 0 ? (
        ...
        standings.map((s, i) => {
          ...
          const canExpand = s.hits.length > 0;
          return (
            <div key={s.name} style={{ marginBottom: 6 }}>
              <div
                onClick={() => canExpand && setExpanded(isOpen ? null : s.name)}
                style={{
                  display: "flex", alignItems: "center", gap: 12, padding: "11px 12px", borderRadius: 10,
                  background: i === 0 && s.pts > 0 ? C.goldPale : s.name === me ? "#E8EBF5" : "transparent",
                  border: `1px solid ${i === 0 && s.pts > 0 ? "#EAD9AC" : s.name === me ? C.line : "transparent"}`,
              ...
              >
                ...
                {canExpand && (
                  <span style={{ fontSize: 11, color: C.inkSoft, width: 12 }}>{isOpen ? "▴" : "▾"}</span>
                )}
              </div>
              {isOpen && (
                <div style={{
                  ...
                  background: "#FAFBFD", padding: "8px 12px",
                }}>
                  {s.hits.map((h, idx) => (
                    <div key={idx} style={{
                      ...
                      <span style={{ color: C.inkSoft }}>
                        {h.match.home} – {h.match.away}
                      </span>
                      <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        <span style={{ fontSize: 11, color: C.inkSoft }}>
                          {h.match.result.score || ""}
                        </span>
                        <span style={{ fontWeight: 700, color: h.ok ? C.green : C.red, minWidth: 40, textAlign: "right" }}>
                          {h.pick} {h.ok ? "✓ +1" : "✗"}
                        </span>
                      </span>
                    </div>
...
function FunFactsTab({ funFacts, allMatches, usersData, users, me, onAdd, onRemove }) {
  const [text, setText] = useState("");
  const stats = computeFunStats(allMatches, usersData, users);
  return (
    <>
      <div style={{ ...st.card, background: "linear-gradient(135deg, #FFF7E5, #FFEAD0)", border: "1px solid #EAD9AC" }}>
        <h2 style={st.h2}>✨ Fun Facts</h2>
        <p style={{ fontSize: 13, color: C.inkSoft, margin: "6px 0 0", lineHeight: 1.5 }}>
          Statistici live + ce mai adaugati voi. Joc serios, observatii ne-serioase.
        </p>
      </div>

      {/* Manual fun facts */}
      <div style={st.card}>
        <h3 style={{ ...st.h2, fontSize: 18, marginBottom: 10 }}>Comentariile casei</h3>
        <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
          <input
            style={{ ...st.input, flex: 1 }}
            placeholder="ex: Madalin a pariat doar X pana acum..."
            value={text}
            onChange={(e) => setText(e.target.value)}
            onKeyDown={(e) => { if (e.key === "Enter") { onAdd(text); setText(""); } }}
            maxLength={200}
          />
          <button style={st.btnSmallPrimary} onClick={() => { onAdd(text); setText(""); }}>
            Adauga
          </button>
        </div>
        {funFacts.length === 0 ? (
          <div style={{ fontSize: 13, color: C.inkSoft, fontStyle: "italic" }}>
            Niciun comentariu inca. Fii prima/primul.
          </div>
        ) : (
          funFacts.map((f) => (
            <div key={f.id} style={{
              padding: "10px 12px", marginBottom: 6, background: "#FAFBFD",
              border: `1px solid ${C.line}`, borderRadius: 10, fontSize: 13,
              display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8,
            }}>
              <div style={{ flex: 1 }}>
                <div>{f.text}</div>
                <div style={{ fontSize: 11, color: C.inkSoft, marginTop: 4 }}>— {f.author}</div>
              </div>
              {f.author === me && (
                <button onClick={() => onRemove(f.id)} style={st.btnGhostDanger}>sterge</button>
              )}
            </div>
          ))
        )}
      ...
      {/* Auto stats */}
      <div style={st.card}>
        <h3 style={{ ...st.h2, fontSize: 18, marginBottom: 10 }}>Statistici live</h3>
        {stats.length === 0 ? (
          <div style={{ fontSize: 13, color: C.inkSoft, fontStyle: "italic" }}>
            Inca nu s-au jucat destule meciuri pentru statistici. Reveniti.
          </div>
        ) : (
          stats.map((s, i) => (
            <div key={i} style={{ padding: "10px 0", borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
              <div style={{ fontSize: 11, color: C.inkSoft, letterSpacing: "0.04em", textTransform: "uppercase", fontWeight: 700, marginBottom: 4 }}>
                {s.emoji} {s.title}
              </div>
              <div style={{ fontSize: 14, color: C.ink }}>{s.value}</div>
            </div>
          ))
        )}
      </div>
...
// ── Calculeaza statistici automate din pariuri si rezultate ──
function computeFunStats(allMatches, usersData, users) {
  const stats = [];
  const finished = allMatches.filter((m) => m.result);
  if (finished.length === 0) return stats;

  // Pentru fiecare user, lista de bilete revelate cu meciul atasat
  const userHits = {}; // { user: [{m, pick, ok}] }
  users.forEach((u) => {
    const reveals = (usersData[u] && usersData[u].reveals) || {};
    userHits[u] = finished
      .filter((m) => reveals[m.id])
      .map((m) => ({ m, pick: reveals[m.id], ok: reveals[m.id] === m.result.pick }));
  });

  // 1. Streak cel mai lung
  let bestStreak = { user: null, len: 0 };
  Object.entries(userHits).forEach(([u, hits]) => {
    let cur = 0, max = 0;
    hits.forEach((h) => {
      if (h.ok) { cur++; max = Math.max(max, cur); } else cur = 0;
    });
    if (max > bestStreak.len) bestStreak = { user: u, len: max };
  });
  if (bestStreak.len >= 2) {
    stats.push({ emoji: "🔥", title: "Cel mai lung streak", value: `${bestStreak.user} cu ${bestStreak.len} pronosticuri corecte la rand.` });
  }

  // 2. Profilul de pariere
  Object.entries(userHits).forEach(([u, hits]) => {
    if (hits.length < 3) return;
    const tally = { "1": 0, "X": 0, "2": 0 };
    hits.forEach((h) => { tally[h.pick] = (tally[h.pick] || 0) + 1; });
    const total = hits.length;
    const dominant = Object.entries(tally).sort((a, b) => b[1] - a[1])[0];
    if (dominant[1] / total >= 0.6) {
      const labels = { "1": "gazda castiga", "X": "egal", "2": "oaspetii castiga" };
      stats.push({
        emoji: dominant[0] === "X" ? "🤝" : dominant[0] === "1" ? "🏠" : "✈️",
        title: `${u} are un profil`,
        value: `${Math.round((dominant[1] / total) * 100)}% din pariuri sunt "${dominant[0]}" (${labels[dominant[0]]}).`,
      });
    }
  });

  // 3. Underdog hero: meci la care doar un singur user a nimerit din mai multi
  finished.forEach((m) => {
    const betters = users
      .map((u) => ({ u, pick: ((usersData[u] && usersData[u].reveals) || {})[m.id] }))
      .filter((x) => x.pick);
    if (betters.length < 3) return;
    const correct = betters.filter((x) => x.pick === m.result.pick);
    if (correct.length === 1 && betters.length - 1 >= 2) {
      stats.push({
        emoji: "🦸",
        title: `${correct[0].u} a fost singura/singurul care a crezut`,
        value: `La ${m.home} – ${m.away}, ${correct[0].u} a pariat ${correct[0].pick}. Restul (${betters.length - 1}) au gresit.`,
      });
    }
  });

  // 4. Surpriza casei: meci la care toti au pariat la fel si s-au inselat
  finished.forEach((m) => {
    const betters = users
      .map((u) => ({ u, pick: ((usersData[u] && usersData[u].reveals) || {})[m.id] }))
      .filter((x) => x.pick);
    ...
    if (wrong.length === betters.length) {
      const pickSet = new Set(betters.map((x) => x.pick));
      if (pickSet.size === 1) {
        stats.push({
          emoji: "🤯",
          title: "Surpriza casei",
          value: `La ${m.home} – ${m.away}, toti cei ${betters.length} au pariat ${betters[0].pick} si toti au gresit.`,
        });
      }
    }
  });

  // 5. Cea mai mare absenta
  const userPlays = users.map((u) => ({ u, played: userHits[u].length }));
  if (userPlays.length >= 2 && finished.length >= 3) {
    const minPlayed = userPlays.sort((a, b) => a.played - b.played)[0];
    const maxPlayed = userPlays[userPlays.length - 1];
    if (maxPlayed.played - minPlayed.played >= 3) {
      stats.push({
        emoji: "🛋",
        title: "Cel mai distrat",
        value: `${minPlayed.u} a pariat la ${minPlayed.played} din ${finished.length} meciuri terminate.`,
      });
    }
  }

  // Limit to top ~6 to avoid overload
...
function AdminPanel({ groupMatches, onSaveGroups, allMatches, koResults, onSaveKo, overrides, onOverride, onResetBets, usersCount }) {
  ...
  // Meciuri care au inceput dar nu au fost inchise
  const unclosed = allMatches.filter((m) =>
    !m.isPlaceholder && !m.result && new Date(m.kickoff).getTime() + 2 * 60 * 60 * 1000 <= Date.now()
  );

  return (
    <>
      {unclosed.length > 0 && (
        <div style={{ ...st.card, background: C.redPale, border: `2px solid ${C.red}` }}>
          <div style={{ fontSize: 14, color: C.red, fontWeight: 700, marginBottom: 6 }}>
            ⚠ {unclosed.length} {unclosed.length === 1 ? "meci terminat dar neinchis" : "meciuri terminate dar neinchise"}
          </div>
          <div style={{ fontSize: 12, color: C.ink, lineHeight: 1.5 }}>
            Aceste meciuri s-au terminat probabil, dar nu ai inregistrat inca rezultatul.
            Pana nu le inchizi, nu se acorda puncte si bracketul nu avanseaza.
          </div>
          <ul style={{ margin: "8px 0 0", paddingLeft: 18, fontSize: 12 }}>
            {unclosed.map((m) => (
              <li key={m.id} style={{ marginBottom: 2 }}>
                <strong>{m.home} – {m.away}</strong> ({m.group || phaseLabel(m.phase)}, {fmtDate(m.kickoff)})
              </li>
            ))}
          </ul>
        </div>
      ...
      <ResetBetsCard onResetBets={onResetBets} usersCount={usersCount} />
    </>
  );
}

function ResetBetsCard({ onResetBets, usersCount }) {
  const [step, setStep] = useState(0); // 0 = initial, 1 = confirm
  if (step === 0) {
    return (
      <div style={st.card}>
        <h2 style={st.h2}>Reseteaza pariurile</h2>
        <p style={{ fontSize: 13, color: C.inkSoft, margin: "8px 0 14px", lineHeight: 1.5 }}>
          Sterge biletele tuturor colegilor si pune clasamentul la 0. Rezultatele meciurilor
          introduse de tine (Mexic 2-0, Canada-Bosnia 1-1, etc.) raman intacte. Foloseste-l
          dupa ce ati testat impreuna si vreti sa porniti de la zero la momentul real.
        </p>
        <button style={{ ...st.btnSmall, color: C.red, borderColor: C.red }} onClick={() => setStep(1)}>
          Reseteaza pariurile ({usersCount} {usersCount === 1 ? "coleg" : "colegi"})
        </button>
      </div>
    );
  }
  return (
    <div style={{ ...st.card, background: C.redPale, border: `2px solid ${C.red}` }}>
      <h2 style={{ ...st.h2, color: C.red }}>Esti sigura?</h2>
      <p style={{ fontSize: 13, color: C.ink, margin: "8px 0 14px", lineHeight: 1.5 }}>
        Vor disparea TOATE biletele puse pana acum, pentru toata lumea. Actiunea nu se
        poate anula.
      </p>
      <div style={{ display: "flex", gap: 8 }}>
        <button style={{ ...st.btnSmall }} onClick={() => setStep(0)}>Anuleaza</button>
        <button style={{ ...st.btnSmallPrimary, background: C.red }} onClick={() => { onResetBets(); setStep(0); }}>
          Da, sterge pariurile
        </button>
      </div>
    </div>
