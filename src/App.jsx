    ...
    <Shell>
      <header style={{ background: `linear-gradient(135deg, ${C.navyDark}, ${C.navy})`, color: "#fff", padding: "18px 16px 0" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <nav style={{ display: "flex", gap: 4, marginTop: 10, overflowX: "auto" }}>
            {[...PHASES, { id: "clasament", label: "🏆 Clasament" }, { id: "funfacts", label: "✨ Fun Facts" }, ...(isAdmin ? [{ id: "admin", label: "⚙ Arbitru" }] : [])].map((p) => (
      ...
      <main style={{ maxWidth: 760, margin: "0 auto", padding: "20px 16px 60px" }}>
        {tab === "clasament" ? (
          ...
          <FunFactsTab allMatches={allMatches} usersData={usersData} users={users} />
        ) : tab === "admin" ? (
          <AdminPanel
...
function MatchCard({ match, me, myPicks, usersData, users, now, onPick }) {
  ...
  const lockAt = placeholder ? Infinity : new Date(match.kickoff).getTime() - 60 * 60 * 1000;
  const started = !placeholder && lockAt <= now;
  ...
  // ALERTA: mai e o ora pana se inchid pariurile si NU ai votat inca
  const msToLock = lockAt - now;
  const showUrgentBanner = !done && !started && !placeholder && !alreadyBet && msToLock > 0 && msToLock <= 60 * 60 * 1000;
  const urgentMins = Math.floor(msToLock / 60000);
  const urgentSecs = Math.floor((msToLock % 60000) / 1000);
        ...
        <>
          {showUrgentBanner && (
            <div style={{
              ...
              boxShadow: `0 0 0 3px ${C.redPale}`,
            }}>
              <span style={{ fontSize: 13, fontWeight: 700 }}>
                ⏰ Mai ai {urgentMins > 0 ? `${urgentMins} min ` : ""}{String(urgentSecs).padStart(2, "0")} sec sa votezi!
              </span>
            </div>
          )}
...
function FunFactsTab({ allMatches, usersData, users }) {
    ...
    <>
      <div style={{ ...st.card, background: "linear-gradient(135deg, #FFF7E5, #FFEAD0)", border: "1px solid #EAD9AC" }}>
        ...
        <p style={{ fontSize: 13, color: C.inkSoft, margin: "6px 0 0", lineHeight: 1.5 }}>
          Statistici live calculate din pariurile si rezultatele voastre.
        </p>
      </div>

      <div style={st.card}>
        {stats.length === 0 ? (
          <div style={{ fontSize: 13, color: C.inkSoft, fontStyle: "italic", textAlign: "center", padding: 20 }}>
