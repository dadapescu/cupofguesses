...
export default function App() {
  ...
  const placeBet = async (matchId, pick) => {
    ...
    const myCommits = { ...((usersData[me] && usersData[me].commits) || {}), [matchId]: true };
...
function NextMatchBanner({ match, now }) {
  ...
  let countdown;
  if (days > 0) countdown = `${days}z ${hours}h ${mins}m`;
  else if (hours > 0) countdown = `${hours}h ${String(mins).padStart(2, "0")}m`;
  else countdown = `${mins}:${String(secs).padStart(2, "0")}`;
  ...
  return (
    <div style={{
      ...
      <span style={{
        fontFamily: "'Bitter', Georgia, serif", fontSize: 20, fontWeight: 800,
      ...
      {!done && !placeholder && (
        <>
          ...
          <div style={{ marginTop: 10, fontSize: 12, color: C.inkSoft, display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
            ...
            {myPick && <span style={{ color: C.green, fontWeight: 700 }}>🔒 Biletul tau: {myPick} (sigilat)</span>}
          </div>
        </>
      )}
      {placeholder && (
...
function Shell({ children }) {
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.ink, fontFamily: "'Inter', -apple-system, 'Segoe UI', sans-serif" }}>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Bitter:wght@600;700;800&family=Inter:wght@400;600;700;800&display=swap');
...
const st = {
  h1: { fontFamily: "'Bitter', Georgia, serif", fontSize: 34, fontWeight: 800, letterSpacing: "0.01em", margin: "14px 0 0", textTransform: "uppercase" },
  h2: { fontFamily: "'Bitter', Georgia, serif", fontSize: 22, fontWeight: 800, textTransform: "uppercase", letterSpacing: "0.02em", margin: 0, color: C.navy },
